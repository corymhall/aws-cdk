...
import { transitGatewayArnComponents } from './private/example-resource-common';

/**
 * The interface that represents the TransitGateway resource.
 ...
 *     {@link TransitGateway} in our case, which implements {@link ITransitGateway}.
 ...
 *     in our case, the {@link TransitGateway.fromTransitGatewayName} method.
 ...
 *
 *  So, an interface like {@link ITransitGateway} represents a resource that *might* be mutable,
 *  while the {@link TransitGateway} class represents a resource that definitely is mutable.
 *  Whenever a type that represents this resource needs to referenced in other code,
 *  you want to use {@link ITransitGateway} as the type, not {@link TransitGateway}.
 *
 ...
 *  in our example, those are {@link transitGatewayArn} and {@link transitGatewayName}.
 *
 ...
 */
export interface ITransitGateway extends
...
/**
 * A common abstract superclass that implements the {@link ITransitGateway} interface.
 * We often have these classes to share code between the {@link TransitGateway}
 * class and the {@link ITransitGateway} instances returned from methods like
 * {@link TransitGateway.fromTransitGatewayName}.
...
abstract class TransitGatewayBase extends core.Resource implements ITransitGateway {
  ...
  /** Implement the {@link ITransitGateway.metricCount} method. */
  public metricCount(metricName: string, props?: cloudwatch.MetricOptions): cloudwatch.Metric {
    return new cloudwatch.Metric({
      ...
      dimensions: { TransitGateway: this.transitGatewayName },
      metricName,
      ...props,
    }).attachTo(this);
  }
...
/**
 * Construction properties for {@link TransitGateway}.
 ...
 * This is this struct for the {@link TransitGateway} class.
 *
...
export interface TransitGatewayProps {
   ...
   * because of the @resource annotation on the {@link TransitGateway} class
...
/**
 * The actual L2 class for the TransitGateway.
 * Extends TransitGatewayBase.
 * Represents a resource completely managed by the CDK, and thus mutable.
 * You can add additional methods to the public API of this class not present in {@link ITransitGateway},
 * although you should strive to minimize that as much as possible,
 * and have the entire API available in {@link ITransitGateway}
 * (but perhaps some of it not having any effect,
 * like {@link ITransitGateway.addToRolePolicy}).
 *
...
export class TransitGateway extends TransitGatewayBase {
  ...
  public static fromTransitGatewayName(scope: core.Construct, id: string, transitGatewayName: string): ITransitGateway {
    ...
    class Import extends TransitGatewayBase {
      ...
      public readonly transitGatewayName = transitGatewayName;
      // Since we have the name, we have to generate the ARN,
      // using the Stack.formatArn helper method from the core library.
      // We have to know the ARN components of TransitGateway in a few places, so,
      // to avoid duplication, extract that into a module-private function
      public readonly transitGatewayArn = core.Stack.of(scope)
        .formatArn(transitGatewayArnComponents(transitGatewayName));
    }

    return new Import(scope, id);
  }

  // implement all fields that are abstract in TransitGatewayBase
  public readonly transitGatewayArn: string;
  public readonly transitGatewayName: string;
  ...
  constructor(scope: core.Construct, id: string, props: TransitGatewayProps = {}) {
    ...
    this.transitGatewayName = this.getResourceNameAttribute(
    ...
    // by writing to the _connections field in TransitGatewayBase,
